...
function ProjectIssuesPage(){
    ...
    async function loadIssues() {
        ...
        if (priorityFilter !== '') {
            queryParameters.set('priority', priorityFilter)
        }
        ...
        try {
            ...
            const issueUrl = `${API_BASE_URL}/projects/${projectId}/issues` + (queryString === '' ? '' : `?${queryString}`)
    ...
    loadIssues()
    }, [projectId, typeFilter, priorityFilter, statusFilter])
    ...
    async function handleDeleteIssue(issueId:string) {
        ...
        try{
            ...
            setIssueActionError('Could not delete issue')
        }
    }
    ...
    return(
        ...
        {isIssueLoading && <p>Loading issues...</p>}
        {issueLoadError && (
            <p className="error-message">{issueLoadError}</p>
        )}
        ...
        setIssues(currentIssues => {
            if (!matchesCurrentFilters(updatedIssue)) {
        ...
        {!isIssueLoading && !issueLoadError && issues.length === 0 &&(
